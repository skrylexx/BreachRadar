# Cahier des Charges Technique — BreachRadar
...
**Endpoints API utilisés dans BreachRadar :**
...
GET /api/v1/victim?name={search_term}
  ...
  → C'est l'endpoint principal utilisé par BreachRadar
...
**Stratégie de matching dans BreachRadar :**

La recherche ne se fait pas uniquement sur le nom de domaine exact — l'organisation peut être listée sous son nom commercial, ses filiales, ou son acronyme. BreachRadar applique une stratégie de matching en cascade :
...
Le `docker-compose.yml` de BreachRadar intègre RansomLook comme service dépendant :
...
# 6. Démarrer la stack complète (BreachRadar + RansomLook)
docker-compose up -d
```
...
```markdown
# 🔍 Rapport BreachRadar — mondomaine.fr
...
- **Tor via RansomLook** : le trafic vers les sites .onion passe par le container Tor dédié — BreachRadar lui-même ne fait aucune connexion Tor directe.
...
├── docker-compose.yml         # BreachRadar + RansomLook (app + redis + tor)
├── Dockerfile
│
├── breachradar/               # Package principal
│   ├── __init__.py
...
RansomLook tourne dans Docker et doit être démarré avant BreachRadar.
...
## Démarrage complet (BreachRadar + RansomLook)
...
uv run python -m breachradar sources --status
...
Procédure :
  ...
  3. Créer une nouvelle application (nom : BreachRadar, plateforme : Other)
...
Procédure de démarrage :
  ...
  2. Le fichier docker-compose.yml fourni avec BreachRadar inclut
  ...
  6. Ajouter dans .env :
       ...
       Note : Si vous utilisez docker-compose (BreachRadar dans Docker),
       l'URL doit être :
...
test:
	uv run pytest tests/unit/ -v --cov=breachradar --cov-report=html
...
- [ ] Conteneurisation Docker complète (BreachRadar + RansomLook en un seul `docker-compose up`)
- [ ] Export PDF
...
*Ce document constitue la spécification technique complète du projet BreachRadar.  
