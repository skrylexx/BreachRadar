...
from app.schemas.cve import CVEAlert as CVEAlertSchema, CVESettings, CVESourceStatus, CVESource
...
def _get_mock_cve_alerts(limit: int, offset: int) -> PaginatedResponse[CVEAlertSchema]:
    from app.schemas.cve import CVESeverity, CVESource
    mock_items = []
    for i in range(limit):
        mock_items.append(CVEAlertSchema(
            ...
            published_at=datetime.now(timezone.utc) - timedelta(hours=i * 5)
        ))
