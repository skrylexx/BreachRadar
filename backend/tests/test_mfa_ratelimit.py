...
async def test_mfa_verify_rate_limiting(async_client):
    ...
    with patch("app.routers.auth.verify_mfa_challenge", new_callable=AsyncMock) as mock_verify:
        ...
        with patch("app.routers.auth.get_mfa_failures", new_callable=AsyncMock) as m_get, \
             patch("app.routers.auth.increment_mfa_failures", new_callable=AsyncMock) as m_inc:
